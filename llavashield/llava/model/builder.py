...
def load_models(model_path, load_8bit=False, load_4bit=False, device_map="cuda", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, **kwargs):
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_path) #tokenizers
    ...
    if isinstance(getattr(cfg, "text_config", None), dict):
        cfg.text_config = PretrainedConfig.from_dict(cfg.text_config)
    if isinstance(getattr(cfg, "vision_config", None), dict):
        cfg.vision_config = PretrainedConfig.from_dict(cfg.vision_config)
    

    model = LlavaQwenForCausalLM.from_pretrained(model_path,config=cfg,low_cpu_mem_usage=True,attn_implementation=attn_implementation,**kwargs)
    model.to(device="cuda", dtype=kwargs["torch_dtype"])
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                from ..model.language_model.llava_mixtral import LlavaMixtralConfig
                ...
                from ..model.language_model.llava_gemma import LlavaGemmaConfig
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    from ..model.language_model.llava_qwen_moe import LlavaQwenMoeConfig
                    if overwrite_config is not None:
