...
class ModelArguments:
    ...
    lora_final_save_dir: Optional[str] = field(default="facebook/opt-125m")
...
class DataArguments:
    ...
    val_data_path: str = field(default=None, metadata={"help": "Path to the training data, in llava's instruction.json format. Supporting multiple json files via /path/to/{a,b,c}.json"})
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    """
    Preprocesses conversation samples for training: adds markers, concatenates, tokenizes, and creates labels with human input masked.
    """
    ...
    return dict(input_ids=input_ids, labels=targets) #[1,L]
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            sources = [sources] # wrap to a list
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME

        if "image" in sources[0]:
            image_file = self.list_data_dict[i]["image"]
            if type(image_file) is list:
                image = [self.process_image(f) for f in image_file] # TODO
        ...
        data_dict = preprocess(sources, self.tokenizer, has_image=has_image) #input_ids:[1,L] labels:[1,L]
        ...
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0]) # [L]
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        return batch #dict(input_ids=[B, L], labels=[B, L], attention_mask=[B, L], images=(N, P, C, H, W)) 
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    val_dataset=None
    if hasattr(data_args, "val_data_path") and data_args.val_data_path is not None:
        val_dataset= LazySupervisedDataset(tokenizer=tokenizer, data_path=data_args.val_data_path, data_args=data_args)
...
def train(attn_implementation=None):
    ...
    trainer = LLaVAShieldTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
    ...
    config_revise(training_args.output_dir)
    ...
    #             merge_lora(model_path=training_args.output_dir,model_base=model_args.model_name_or_path,save_model_path=model_args.lora_final_save_dir)

    #             rank0_print("LoRA adapters saved.")
